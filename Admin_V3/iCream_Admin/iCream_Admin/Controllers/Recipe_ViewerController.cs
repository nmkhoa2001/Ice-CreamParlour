...
namespace iCream_Admin.Controllers
{
    public class Recipe_ViewerController : Controller
    {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "recipeV_id,caterory_name,recipeV_nameViewer,recipeV_name,recipeV_price,recipeV_img,recipeV_imgFile,recipe_status,recipeV_text")] Recipe_Viewer recipe_Viewer)
        {
            if (recipe_Viewer.recipeV_imgFile.ContentLength > 0)
            {
                string fileExtension = Path.GetExtension(recipe_Viewer.recipeV_imgFile.FileName);
